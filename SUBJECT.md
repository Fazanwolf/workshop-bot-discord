...
Pour ce faire, créez une nouvelle application sur le Portail Développeur, nommez votre application et acceptez les
clauses.

#hint([Portail Développeur](https://discord.com/developers/applications))

#hint(Sur l'onglet `General Information`, vous pouvez customiser votre application (nom, description, icon de profil, etc))

### Transformer une Application en Bot

Pour pouvoir invitez votre application sur votre serveur discord de test, vous devez la définir comme bot.
...
Pour ce faire, sur l'onglet Bot, dans `Privileged Gatewy Intent` ajoutez les Gateway que vous souhaitez.

#warn(Ne partagez surtout pas votre `BOT_TOKEN`)

### Ajouter votre bot à un serveur

Maintenant que votre bot est paramètré, il faut l'ajouter à un serveur.

Sur l'onglet `OAuth2`, dans le sous-onglet `URL Generator`, sélectionnez la méthode `In-app Authorization`, cochez `bot`
et ajoutez vos permissions.

#warn(Un bon bot est un bot avec des permissions clairement définis.)
...
- GUILD_ID: [Developer Mode](https://beebom.com/how-enable-disable-developer-mode-discord/) > Clique droit sur le serveur où se trouve votre bot > `Copy ID`
...
#hint(Cherchez comment faire un objet et des arrays par vous-même)
...
#warn(Attention aux évènements qui doivent être exécuter qu'une seule fois)
...
- Le type d'interaction corresponds à celui d'une `SlashCommand`, retourner si ce n'est pas le cas.
...
#hint(Félicitation, vous avez réussi à faire un bot trash mais ce n'est que le début du workshop.)
...
On va gérer nos différentes fonctions avec une enumération `InteractionType` qui aura les types d'interaction.
...
#hint(Une [`Collection`](https://discord.js.org/#/docs/collection/main/general/welcome) est une [`Map`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Array/map) implémenter pour Discord)
...
Dans le fichier `click-me.bouton.js`, vous allez créer une `data` de type `Button` qui aura les même informations que le boutton de la commande `click-me.slash.js` et un `execute` qui répondra avec une image.
...
Amusez-vous à créer des intéractions de type `Selection` et `Modal`.
